...
export function listCredentialSources(authDir, deps = {}) {
  ...
  for (const entry of entries) {
    ...
    sources.push({ ...described, mtimeMs: statSync(filePath).mtimeMs })
  }
  // 按 uid 去重：同一账号可能有多份凭据文件（如桌面端的时间戳备份
  // workbuddy-desktop.<timestamp>.info），过期备份与正式文件并列会让用户看到
  // 两个一模一样的按钮，且点进过期备份必然 401。
  //
  // 保留策略以「文件修改时间」为主，**不是** expiresAt：备份文件里的 expiresAt 可能是
  // 伪造的远期值（真实回归：备份写 2027，token 实际早已被服务端作废，切过去一直 401），
  // 而正式文件的 mtime 永远晚于它的历史备份。mtime 同分时才比较 expiresAt。
  const byUid = new Map()
  // mtime 抖动容差：同批创建的文件毫秒级差异无意义，2 秒内视为同分，回退到 expiresAt 比较。
  const MTIME_TIE_MS = 2000
  for (const source of sources) {
    const uidKey = `${source.label.uidTail}\u0000${source.label.phoneMasked}\u0000${source.label.nickname}`
    const existing = byUid.get(uidKey)
    if (existing === undefined) {
      byUid.set(uidKey, source)
      continue
    }
    let keepCandidate
    if (Math.abs(source.mtimeMs - existing.mtimeMs) <= MTIME_TIE_MS) {
      keepCandidate = source.accessExpiresAt > existing.accessExpiresAt
    } else {
      keepCandidate = source.mtimeMs > existing.mtimeMs
    }
    if (keepCandidate) byUid.set(uidKey, source)
  }
  // 已过期账号直接剔除：它们不可用（推理必 401），留在可切换列表只会误导用户。
  // expiresAt=0（上游没给）不算过期——无法判断时保守保留。
  const now = Date.now()
  const deduped = [...byUid.values()].filter(
    (source) => !(source.accessExpiresAt > 0 && source.accessExpiresAt <= now),
  )
  deduped.sort((a, b) => a.label.nickname.localeCompare(b.label.nickname, 'zh-CN'))
  return deduped.map(({ mtimeMs, ...rest }) => rest)
}
