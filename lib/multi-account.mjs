...
export function listCredentialSources(authDir, deps = {}) {
  ...
  // 两个一模一样的按钮，且点进过期备份必然 401。保留策略：
  //   1. 未过期优先于已过期；2. accessExpiresAt 更大者；3. 同分时后者（目录序更晚）让位。
  const byUid = new Map()
  for (const source of sources) {
    ...
    const now = Date.now()
    const existingExpired = existing.accessExpiresAt > 0 && existing.accessExpiresAt <= now
    const candidateExpired = source.accessExpiresAt > 0 && source.accessExpiresAt <= now
    let keepCandidate
    if (existingExpired !== candidateExpired) keepCandidate = !candidateExpired
    else keepCandidate = source.accessExpiresAt > existing.accessExpiresAt
