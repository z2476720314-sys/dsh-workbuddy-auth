...
/** Host 内部用的多账号载体；与 CREDIT_ENVELOPE 同型。 */
const ACCOUNTS_ENVELOPE = Symbol('dsh-workbuddy-auth.accounts')

/**
 * 把多账号数据挂在 String 包装对象上（与 withCredits 同型，可叠加）。
 * sources 必须是 Host 侧已白名单化的脱敏列表；buildStatus 侧再做一道防御性过滤。
 */
export function withAccounts(rawText, accounts) {
  const envelope = rawText?.[CREDIT_ENVELOPE] !== undefined ? rawText : new String(rawText)
  Object.defineProperty(envelope, ACCOUNTS_ENVELOPE, { value: accounts, enumerable: false })
  return envelope
}

/** 多账号 DTO 的防御性白名单：即使 Host seam 返回脏对象，也只允许安全字段进入响应。 */
function safeAccounts(accounts) {
  if (accounts?.enabled !== true) return { enabled: false, activeId: '', sources: [] }
  const sources = (Array.isArray(accounts.sources) ? accounts.sources : []).slice(0, 32).map((source) => ({
    id: typeof source?.id === 'string' ? source.id.slice(0, 64) : '',
    label: {
      nickname: typeof source?.label?.nickname === 'string' ? source.label.nickname.slice(0, 64) : '',
      uidTail: typeof source?.label?.uidTail === 'string' ? source.label.uidTail.slice(0, 8) : '',
      phoneMasked: typeof source?.label?.phoneMasked === 'string' ? source.label.phoneMasked.slice(0, 16) : '',
    },
    accessExpiresAt: Number.isFinite(Number(source?.accessExpiresAt)) ? Number(source.accessExpiresAt) : 0,
  }))
  return {
    enabled: true,
    activeId: typeof accounts.activeId === 'string' ? accounts.activeId.slice(0, 64) : '',
    sources,
  }
}
...
export function buildStatus(input = {}) {
  ...
  const accountsEnvelope = rawText?.[ACCOUNTS_ENVELOPE]
  ...
  const accounts = safeAccounts(accountsEnvelope)
  ...
  if (failure === undefined) {
    try {
      ...
      return finalizeCredits({ ok: true, account, token, state, accounts }, credits)
    } catch (caught) {
  ...
  return finalizeCredits({ ok: false, account, token, state, accounts, error: message }, credits)
}
