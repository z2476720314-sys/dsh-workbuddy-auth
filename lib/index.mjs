 ...
 */
export function createRouteDeps(ctx, config = {}, fetchImpl = fetch, syncDeps = createSyncDeps(ctx, config, fetchImpl), activeFile) {
  const staticFile = config.credentialFile || DEFAULT_CREDENTIAL_FILE
  const currentFile = () => (activeFile ? activeFile.file() : staticFile)
  ...
  return {
    ...
    /**
     * 多账号（config.authDir 启用时存在）：账号发现 + 切换。
     * describe 返回脱敏列表；activate 切换内存激活文件 → 持久化 → 立即重载 seam。
     */
    ...(authDir !== ''
      ? {
          ...
          activateCredentialSource: async (id) => {
            ...
            void persistCredentialFileToPatch(config.patchFile, activeFile.file())
            return { ok: true, activeId: result.activeId, persisted: result.persisted }
          },
        }
      : {}),
  }
...
function resolveActiveCredentialFile(config, authDir = resolveAuthDir(config)) {
  ...
  return { file: () => current, update: (path) => { current = path }, selector }
}
...
export function apply(ctx, config = {}) {
  ...
  // 多账号：选择器的 activate 需要「切换内存引用 + 立即重载 seam + 持久化 patch」。
  // reloadSeam 直接复用 apply 的同步管线（受同一重入保护），persist 写回用户 patch。
  if (activeFile.selector !== undefined) {
    activeFile.selector.activate = async (id) => {
      const { readdirSync: readdir } = await import('node:fs')
      let targetPath = ''
      const { join: joinPath } = await import('node:path')
      for (const entry of readdir(authDir, { withFileTypes: true })) {
        if (!entry.isFile() || !entry.name.toLowerCase().endsWith('.info')) continue
        const filePath = joinPath(authDir, entry.name)
        // id 校验在 createAccountSelector.activate 内完成；这里把 id 翻译回路径。
        const { createHash } = await import('node:crypto')
        if (createHash('sha256').update(filePath).digest('hex').slice(0, 16) === id) { targetPath = filePath; break }
      }
      if (targetPath === '') throw new Error(`unknown credential source: ${id}`)
      const probeText = await readFile(targetPath, 'utf8')
      if (typeof probeText !== 'string' || probeText === '') throw new Error(`unknown credential source: ${id}`)
      activeFile.update(targetPath)
      // 立即重载 seam：复用 runSync 的同一管线的重入保护（routeDeps.reloadSeam 也走这条）。
      await runSync(deps, ctx.logger, 'account-switch')
      const persisted = await persistCredentialFileToPatch(config.patchFile, targetPath)
      return { ok: true, activeId: id, persisted }
    }
  }
  ...
  const routeDeps = createRouteDeps(ctx, config, fetch, deps, activeFile)
